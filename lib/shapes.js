...
class Square extends Shape {
    ...
    render() {
        return `
        <svg version="1.1" width="300" height="300" xmlns="http://www.w3.org/2000/svg">
        <rect x="50" y="50" width="250" height="250" fill="${this.shapeColor}"/>
        <text x="175" y="200" font-size="100" text-anchor="middle" fill="${this.textColor}">${this.text}</text>
        </svg>
        `
    };
};
...
class Triangle extends Shape {
    ...
    render() {
        ...
        <polygon points="225 50, 400 350, 50 350" fill="${this.shapeColor}"/>
        <text x="225" y="300" font-size="100" text-anchor="middle" fill="${this.textColor}">${this.text}</text>
        </svg>
        `
    };
};
